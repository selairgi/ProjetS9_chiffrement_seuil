    ...
    def initiate_encryption(self, messages: List[bytes], server_ids: List[int]) -> Dict[str, Any]:
        ...
        for idx, resp in enumerate(responses):
            ...
            stmt = HiseEncNizkStatement(
                ...
                h=self.proof_params.h,
                h_of_x_eps=h_root,
                # h_of_x_eps_pow_a est ce que le serveur a mis dans la proof
                # dans la fonction prove(...), il l'a mis en param => alpha_share
                # => On doit le reproduire ici: alpha_share = combined_share / (h_x_w^β_i).
                #    On ne connait pas β_i => c'est normal.
                # Pour la validation, on va se baser sur la structure interne du proof.
                # Cf. proof_enc.verify(...) => on s'attend à stmt.h_of_x_eps_pow_a
                #    comme "h_root^α_i".
                # => Le proof stocke déjà 'ut1' / 'ut2', on a besoin d'un 'stmt' cohérent.
                # On peut, par simplification, remettre combined_share à la place, mais
                # ce n'est pas parfaitement "sémantiquement" exact. 
                # On se fie ici au fait que proof_enc.verify(stmt) est identique
                # à ce qui est fait dans le code source (nizk.py).
                # h_of_x_eps_pow_a=combined_share,
